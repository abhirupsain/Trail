...
change_indices = all_indices[udiff != 0]
...
if len(change_indices) % 2 == 1:
    change_indices = change_indices[:-1]  # omit last value
...
change_indices = change_indices.reshape(-1, 2) # -1  means: "choose the row number such that it fits"
...
for idx1, idx2 in change_indices:
    ii_mean[idx1:idx2] = np.mean(ii[idx1:idx2]) # Calculate mean values (and save them)


if 0:

    plt.plot(tt, ii)  # current-values with noise
    plt.plot(tt, ii_mean)  # mean values of the current
...
if 0:
    ...
    start_idcs = change_indices[:, 0] # first column: indices where a block (ore section) starts

    # Determine a voltage-current value pair for each current block:
    ii2 = ii_mean[start_idcs]
    uu2 = uu[start_idcs]

    plt.plot(uu2, ii2, 'bx', ms=7)  # big blue crosses (x)

    a1, a0 =  sc.polyfit(uu2, ii2, 1)  # lineare regression

    plt.plot(uu2, a1*uu2+a0, 'g-')  # Evaluate and plot polynomial (straight line equation)
...
if 0:
    for idx1, idx2 in change_indices:
        ...
        plt.plot(xd[idx1:idx2-1], xdd[idx1:idx2-1])
...
def calc_voltage(v, a):
    ...
    return interp_voltage[idx_v, idx_a]*s

###### task 11

# load swingup data
data = np.load('../data/swingup.npy')
